...
export class FillInformComponent implements OnInit {
	...
	constructor(private router: Router, private tip: TipPopService, private http: HttpService, private common: CommonService) { }

	ngOnInit() {
		this.common.getUserPermission(data => {
	        if(data && data['10000101']){
				this.scrollTo();
				this.initTime();
				this.onScroll();	        	
	        }else{
	...
	post() {
		...
		setTimeout(() => {
			...
			if(this.isEasy) {
				...
				if(this.easy.rentPayMode != '---请选择---' && this.easy.rentPayMode != '自定义') {
					params.shopInfo.rentPayMode= this.easy.rentPayMode;
				}else if(this.easy.rentPayMode == '自定义' && this.myRentPayMode){
					params.shopInfo.rentPayMode= this.myRentPayMode;
				}
				if(this.easy.rentDepositMode != '---请选择---' && this.easy.rentDepositMode != '自定义') {
					params.shopInfo.rentDepositMode= this.easy.rentDepositMode;
				}else if(this.easy.rentDepositMode == '自定义' && this.myRentDepositMode){
				...
				if(this.intact.parentPropertyName) {
				...
				if(this.intact.riskDescription && this.intact.removeRisk == 2) {
				...
				if(bestTimesArr.length) {
					let bestList = bestTimesArr.map(v => v.code);
					params.shopInfo.operateBestTimes = bestList.join();
				}
				...
				if(this.employeeList.length){
					...
					this.employeeList.forEach(v => {
						...
						employeeList2.push(v);
					})
					if(employeeList2.length){
						params.employeeList = employeeList2;
					}
				}
				//证件信息
				if(this.certificateList.length){
					let certificateList2 = [];
					this.certificateList.forEach(v => {
						v = JSON.parse(JSON.stringify(v));
						if(v.photoIds && v.photoIds.length){
							v.photoIds = v.photoIds.map(v => v.id);
						}
						certificateList2.push(v);
					})
					if(certificateList2.length){
			...
			this.http.post('shop', params, data => {
				...
				setTimeout(() => {
					this.router.navigate(['/admin/releaseOk/' + data.shopId]);
				}, 500);
			})
